...
public class Navigator {
	...
	public Navigator(int numberOfBooks, Metadata activityM) {
		nBooks = numberOfBooks;
		books = new Manipulator[this.nBooks];
	...
	public Navigator(int numberOfBooks, MainActivity activityMV) {
		nBooks = numberOfBooks;
		books = new Manipulator[nBooks];
		views = new SplitPanel[nBooks];
		extractAudio = new boolean[nBooks];
		activity = activityMV;
	...
	public boolean openBook(String path, int index) {
		try {
			...
			books[index] = new Manipulator(path, index + "", context);
		...
		for (int i = 0; i < nBooks; i++) {
			...
			if (path != null) {
				try {
					...
					try {
						books[i] = new Manipulator(path, i + "", context);
						books[i].goToPage(current);
					} catch (Exception e2) {
						ok = false;
					} catch (Error e3) {
